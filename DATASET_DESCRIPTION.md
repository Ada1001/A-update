...
Use `--model ms_tgc_spddsbn` for the fusion model. Shared multi-scale kernels produce a full temporal feature map for every EEG channel, scale attention fuses the maps, and Chebyshev propagation operates across channels at every time sample. Learned channel pooling yields one graph-temporal sequence whose mean and covariance respectively produce Euclidean graph and SPD tangent-space representations. Training uses source-label cross entropy only. Unlabeled target windows are used only for SPDDSBN statistics in distinct-domain protocols; same-domain within-session refitting is disabled automatically. Target/test labels are never used.

MS_TGC_SPDDSBN ablations are selected as separate `--model` values while sharing the same implementation file and split policy. `mstgc_dta_ce` keeps shared per-channel multi-scale temporal maps with CE. `mstgc_dta_cheb_ce` adds time-resolved Chebyshev propagation. `mstgc_dta_cheb_eudsbn` adds Euclidean domain-specific BN. `mstgc_dta_cheb_spdmbn` and its `mstgc_dta_cheb_spdbn` alias add non-domain-specific SPD manifold BN. `ms_tgc_spddsbn` is the full model. `mstgc_wo_dta` replaces multi-scale attention with one shared temporal scale. `mstgc_wo_cheb` removes graph propagation but retains the shared temporal sequence for SPD covariance. `mstgc_wo_spddsbn` keeps the shared graph-temporal SPD head without manifold BN.
