...
Use `--model mdtn-gmda` for the local MDTN-GMDA baseline. The adapter consumes source-normalized raw 1 s windows, learns multi-scale temporal features, and trains with WarmStartGRL, a Chebyshev graph discriminator, marginal MMD, conditional pseudo-label MMD, and graph matching losses. Source train windows provide class labels; unlabeled target/test windows are used only by these transfer losses when `target_adapt=True`. This is separate from both TSMNet's SPDDSBN target BN refit and BF-GCN's graph adversarial branch. Use `--no-target-adapt` for a source-only MDTN ablation.
