...
Use `--model ms_tgc_spddsbn` for the fusion model. It combines TSMNet-SPDDSBN's SPD manifold branch with MDTN-GMDA's multi-scale temporal feature extractor and Chebyshev graph-convolution feature encoder. Fusion is performed on projected latent features with a learnable gate, not by averaging or appending final logits. The model deliberately removes MDTN-GMDA's domain-adversarial, MMD, and graph-matching losses: training uses only source-label cross entropy. Target/test labels are never used in training, and unlabeled target/test windows are used only to refit the embedded SPDDSBN statistics when `target_adapt=True`. Use `--no-target-adapt` for a strict source-only ablation.

MS_TGC_SPDDSBN ablations are selected as separate `--model` values while sharing the same implementation file and split policy. `mstgc_dta_ce` keeps only the multi-scale temporal DTA branch with CE. `mstgc_dta_cheb_ce` adds Chebyshev graph convolution. `mstgc_dta_cheb_eudsbn` adds ordinary Euclidean domain-specific BN and uses unlabeled target windows only for BN-statistic refitting. `mstgc_dta_cheb_spdmbn` adds the SPD branch with TSMNet's original SPD manifold BN (`spdbn`) under the SPDMBN ablation name. `mstgc_dta_cheb_spdbn` is kept as the original SPDBN-named alias. `ms_tgc_spddsbn` is the full DTA + Cheb + SPDDSBN model. `mstgc_wo_dta` replaces DTA with channel-level temporal summary features before Cheb. `mstgc_wo_cheb` removes the Cheb encoder and fuses DTA features directly. `mstgc_wo_spddsbn` keeps the fusion backbone with a plain TSMNet SPD branch but removes SPDDSBN alignment.
