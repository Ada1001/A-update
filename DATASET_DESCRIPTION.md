...
Use `--model ms_tgc_spddsbn` for the proposed model. Shared multi-scale kernels produce a full temporal feature map for every EEG channel, scale attention fuses the maps, and Chebyshev propagation operates across channels at every time sample. Channel reliability weights rescale but do not sum electrodes. The preserved channel-time observations jointly estimate a mean and shrinkage covariance, which are embedded as one augmented SPD matrix before BiMap, ReEig, SPDDSBN, and tangent-space classification. Training uses source-label cross entropy only. Unlabeled target windows are used only for SPDDSBN statistics in distinct-domain protocols; same-domain within-session refitting is disabled automatically. Target/test labels are never used.

MS_TGC_SPDDSBN ablations are selected as separate `--model` values while sharing the same implementation file and split policy. The CE and Euclidean-DSBN variants use the first-order mean of the same reliability-weighted channel-time observations. Every SPD variant uses the same first/second-order augmented SPD construction. `mstgc_dta_cheb_spdmbn` and its `mstgc_dta_cheb_spdbn` alias use non-domain-specific SPD manifold BN. `ms_tgc_spddsbn` is the full model. `mstgc_wo_dta` replaces multi-scale attention with one shared temporal scale. `mstgc_wo_cheb` removes graph propagation. `mstgc_wo_spddsbn` keeps the augmented SPD head without manifold BN.
