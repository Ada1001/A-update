...
The controlled graph-source variants are `ms_tgc_spddsbn` (source-learned adaptive top-k), `mstgc_graph_prior` (fixed 10-20 spatial prior), `mstgc_graph_plv` (fixed source-train mean theta/alpha/beta/gamma PLV), and `mstgc_graph_multigraph` (spatial plus four PLV graphs with learned mixture weights and a shared Chebyshev layer). PLV construction reads only source training indices after source-fitted normalization; validation and target/test windows do not participate. `--mstgc-graph-k` controls graph sparsity and defaults to 4.
