...
For COG-BCI, `run_experiment.py` applies the same split-quality checks used by `inspect_datasets.py` to all three protocols. Subjects whose train/validation/test split has too few windows or missing/too-small class counts are skipped by default and recorded through the printed skip message; pass `--allow-incomplete-splits` only for an explicitly reported diagnostic run.
...
By default, `spddsbn` uses TSMNet-style unsupervised target-domain adaptation: target/test windows alone are used without labels to refit their domain-specific BN statistics after the source-validation best epoch has been restored. Source windows are not mixed into the target refit call. This is recorded as `target_adapt=True` and `target_refit_scope=target_only` in `summary.csv`. Use `--no-target-adapt` for a stricter no-target-feature baseline.
...
Use `--model mdtn` for the local MDTN-GMDA baseline. The adapter follows `MDTN-GMDA/Net.py` by using multi-scale temporal convolution, dynamic attention, contextual gating, warm-start gradient reversal, a Chebyshev graph discriminator, graph matching, and marginal/conditional MMD. It consumes source-normalized raw 1 s EEG windows. Source train windows provide class labels; target/test labels are never used in training, and unlabeled target/test windows are used only by MDTN-GMDA transfer losses when `target_adapt=True`. Use `--no-target-adapt` for a source-only MDTN-GMDA ablation.

Use `--model ms_tgc_spddsbn` for the proposed model. Shared multi-scale kernels produce a full temporal feature map for every EEG channel, scale attention fuses the maps, and Chebyshev propagation operates across channels at every time sample. Channel reliability weights rescale but do not sum electrodes. The preserved channel-time observations jointly estimate a mean and shrinkage covariance, which are embedded as one augmented SPD matrix before BiMap, ReEig, SPDDSBN, and tangent-space classification. Training uses source-label cross entropy only. Unlabeled target windows are used only for target-only SPDDSBN statistics refitting. In `single_session` this is a same-domain transductive operation; disable it with `--no-target-adapt` for inductive evaluation. Target/test labels are never used for training, model selection, graph construction, normalization fitting, or SPDDSBN refitting.

MS_TGC_SPDDSBN ablations are selected as separate `--model` values while sharing the same implementation file and split policy. The controlled representation group is `mstgc_mean_ce`, `mstgc_cov_spddsbn`, and `mstgc_augspd_spddsbn`; the first and third are explicit aliases of `mstgc_dta_cheb_ce` and `ms_tgc_spddsbn`, respectively. Covariance-only and augmented SPD use identical shrinkage and manifold processing, so their difference isolates first-order mean embedding. Module ablations independently remove multi-scale attention, Chebyshev propagation, learned channel attention, or SPDDSBN. Graph-source variants change only adjacency construction.

The controlled temporal group keeps the graph and augmented-SPD path fixed: `mstgc_wo_dta` is a single shared temporal convolution, `mstgc_temporal_multiscale` uses three branches with fixed equal averaging, `mstgc_temporal_dta` learns the three scale weights with DTA, and `ms_tgc_spddsbn` adds contextual gating. The default 128 Hz kernels are 17/9/5; dataset-aware scaling gives 31/15/7 at 250 Hz. Actual kernels, temporal mode, and parameter count are stored in result CSV files.

The controlled adaptive-graph table is: G1 `mstgc_wo_cheb` (no graph), G2 `mstgc_graph_prior` (fixed 10-20 spatial prior), G3 `mstgc_graph_plv` (fixed source-train mean theta/alpha/beta/gamma PLV), G4 `mstgc_graph_correlation` (fixed source-train mean absolute Pearson correlation), G5 `mstgc_wo_channel_attention` (learnable adaptive top-k graph with uniform channel reliability), and G6 `ms_tgc_spddsbn` (learnable adaptive top-k graph plus channel reliability attention). All six retain the same augmented-SPD/SPDDSBN path; only G1 omits Chebyshev propagation and only G5 omits learned channel reliability. PLV and correlation construction read only source training indices after source-fitted normalization; validation and target/test windows do not participate. `--mstgc-graph-k` controls graph sparsity and defaults to 4. `mstgc_graph_multigraph` remains an additional spatial-plus-four-PLV learned-mixture experiment outside this six-row table.
...
For binary LSCCN experiments, the two capsule lengths are converted to a decision score and a single threshold is selected on the validation split only. The fixed validation threshold is then used for train, validation, and test metrics and is recorded in `summary.csv`; test labels are not used to choose it.
...
Cache files should match the loaded scope. For example, do not reuse a `sub01` COG-BCI cache for a leave-one-subject-out run over all subjects. The loader checks dataset name, requested sampling rate, strict preprocessing metadata, requested sessions, and explicitly requested subjects. For full COG-BCI caches, the cache stores recording subjects and subjects that produced no usable windows. A subject can therefore be absent from evaluation without causing a hard failure when it is a known no-window or split-quality exclusion. If the data root contains new recording subjects that were not visible when the cache was built, rebuild with `--rebuild-cache`. Older strict caches without this COG-BCI coverage metadata are still usable as their own subject universe, but they should be rebuilt once for formal reporting.
