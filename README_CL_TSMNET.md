...
Default splits are leakage-conscious. `single_session` uses contiguous time blocks within each task record with train/validation/test = 70%/10%/20% by default. `cog_multi_session` uses COG-BCI session 1 for training, session 2 for validation, and session 3 for target/test. `loso` leaves one subject as target/test, then randomly selects `ceil(20% * source_subjects)` source subjects for validation by default; the remaining source subjects are used for training.
